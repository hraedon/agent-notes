...
def require_gate_ran(
    ...
    *,
    ...
    Raises ``AssertionError`` with the exit code, parsed counts, and a short
    output fragment so the failure is debuggable. Returns the counts.
    """
...
def test_require_gate_ran_rejects_an_all_skip_summary() -> None:
    ...
    with pytest.raises(AssertionError, match="importorskip"):
        require_gate_ran(
            "s.....                                   [100%]\n5 skipped in 0.5s\n",
            exit_code=0,
        )
