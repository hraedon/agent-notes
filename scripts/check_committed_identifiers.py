...
2. Secret-driven: when ``AGENT_NOTES_FORBIDDEN_IDENTIFIERS`` is set (a
...
def _unconfigured(reason: str) -> None:
    ...
    if _declares_public():
        raise GateError(
            ...
            "AGENT_NOTES_FORBIDDEN_IDENTIFIERS environment variable "
...
def _resolve_identifiers() -> frozenset[str] | None:
    ...
    """
    raw = os.environ.get("AGENT_NOTES_FORBIDDEN_IDENTIFIERS", "")
    if not raw.strip():
        _unconfigured("AGENT_NOTES_FORBIDDEN_IDENTIFIERS is empty or unset")
        return None
    identifiers = parse_identifier_set(raw)
    if not identifiers:
        _unconfigured(
            "AGENT_NOTES_FORBIDDEN_IDENTIFIERS contained no usable identifiers "
