...
| 9b.4 | ~~opencode equivalents of each skill in `skills/opencode/` mirror tree.~~ **Resolved differently (see §9 Q4):** install-time frontmatter rewrite from the single `skills/<name>/SKILL.md`. No mirror tree. | Cross-harness parity. |
...
4. **Skills install path on opencode.** ~~opencode's skill loading conventions are less settled than Claude Code's. Worth confirming the target directory before 9b.4.~~ **Resolved 2026-05-27:** opencode loads commands from `~/.config/opencode/command/<name>.md` with `description:`-only frontmatter (name derives from filename). `install-skills --target opencode` now reads the same `skills/<name>/SKILL.md` files, strips the `name:` frontmatter line, and writes to that path. Decision 54a: single source of truth — no parallel `skills/opencode/` tree. If wording must diverge later, add a `skills/<name>/SKILL.opencode.md` override file; until then, the prose is identical across harnesses.
