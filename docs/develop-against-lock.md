...
`0.7.2`), then `ruff` and `-e ".[test]"` (pytest, testcontainers, and the pinned
...
(`regista-hraedon>=0.7.0,<0.8`). `SUITE.lock` pins the published 0.7.2 artifact,
its `v0.7.2` merge SHA, envelope v6, and canonical workflow v3 together.
...
The current editable `uv.lock` records the sibling checkout's 0.7.2 metadata.
