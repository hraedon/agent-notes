...
- **Lease ops and link audit rows are attributed (WI-069, follow-on to WI-068).** The native (degrade-mode) `claim`/`release`/`heartbeat` verbs now stamp the gate-resolved identity: the op payloads and their change_log rows record the resolved `model_lineage` (the per-invocation declaration when given, the env declaration otherwise), so the op-chain can say *which* lineage held a lease after the fact — the regista-path claim already carries it in `actor_metadata`. With no explicit `--actor-id` (the documented default — the flag is an override of the env-wired identity) the env-resolved actor now lands on the op payloads, op actor columns, change_log rows, and the lease row, instead of `None` / the signer `"null"` substitute / the lease `"unknown"` fallback; an explicit `--actor-id` still wins, and the Plan 010 claim-identity semantics are unchanged. `links.add_link`/`remove_link` change_log rows (`link_added`/`link_removed`) are no longer NULL-actored: they carry the caller-resolved actor (`link-cross` passes its lineage-gated actor; the plain `link add`/`link remove` CLI stamps the env-resolved default). Attribution only — gate/refusal behavior is unchanged. One residual asymmetry to be aware of when reading op chains: `file`/`update`/`close` op payloads still record the *raw* `model_lineage` parameter (`None` under env-only wiring) while the lease ops record the *resolved* value — payload semantics are not yet uniform across op types.
...
- **`agent-notes invariants probe` — the genesis-gate session-identity probe (WI-071).** The suite genesis gate (`agent_suite.genesis_gate`) requires a passing `agent_notes.session_identity_resolvable` check from agent-notes before an epoch may open; until now that probe did not exist and was a named, permanent gate blocker. `agent-notes invariants probe --json` emits the versioned probe report (`probe_version: 1`, namespaced check ids, single JSON document on stdout, exit 0/1 with `(exit == 0) == ok`) and `--help` exposes the `usage: agent-notes invariants probe` line `agent-suite schedule install` preflights. The probe is read-only — no database connection, no regista face, no event — and it measures *this* environment rather than a fixture: it resolves the actor and model lineage through `face_factory.actor_with_overrides`, the entry point every authored work-item write uses, and reports the write path's own verdict. Three checks: `agent_notes.session_identity_resolvable` (required by the gate), `agent_notes.lineage_registry_available`, and `agent_notes.write_path_refuses_unresolvable_lineage` (a deny-case with a positive control, so a passing identity report cannot come from a validator that stopped refusing). Failure modes are named in each check's `reason`. Two deliberate strictnesses: an *unavailable* regista lineage registry is a failure rather than a degradation — with no closed vocabulary to resolve against, "resolvable" is unproven, and an unproven claim reported as `pass` is the fail-open shape a gate probe exists to prevent (the gate separately requires `regista.closed_lineage_registry`, so this blocks no gate opening that could honestly happen) — and a *whitespace-only* `AGENT_NOTES_ACTOR_ID` fails even though `require_declared_lineage` tolerates it (an empty value is not the same thing: it falls through to the `agent-notes` default and passes).
